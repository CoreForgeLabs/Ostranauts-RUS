...
def scan_file(name):
    ...
    for rec_id, field, ru_val in str_fields(ru):
        ...
        #    Помечаем не любой [*-pos], а только тот, чей обладатель совпадает с
        #    подлежащим строки: именно там по-русски обязано стоять возвратное
        #    "свой". Если обладатель другой ("[us] чинит [them-pos] костюм"),
        #    личное местоимение уместно, и находка была бы ложной.
        subject = None
        m_subj = re.match(r"\s*(?:У\s+)?\[([a-zA-Z0-9]+)(?:-(?:subj|gen))?\]", ru_val)
        if m_subj:
            subject = m_subj.group(1)
        if subject and (subject + "-pos") in ru_tokens:
            add("POS_TOKEN", "[%s-pos] при подлежащем [%s]: нужно возвратное 'свой'"
                % (subject, subject))
