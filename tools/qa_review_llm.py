...
- VERB_SWAPPED: перевод не соответствует оригиналу -- при массовом прогоне
  на эту строку заехал текст ЧУЖОЙ реплики. Не чини его, а переведи оригинал
  заново, с нуля, сохранив все токены оригинала.
...
def main():
    ...
    if "--redo" in argv:
        ...
        print("режим --redo: разбираем заново, чекпойнты игнорируются")
    seen, uniq = set(), []
    for f in findings:
